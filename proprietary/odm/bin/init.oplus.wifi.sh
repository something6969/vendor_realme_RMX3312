...
if [ ! -s /mnt/vendor/persist/bdwlanw.elf ] ; then
    cp /odm/etc/wifi/bdwlanw.elf /mnt/vendor/persist/bdwlanw.elf
    sync
fi
...
if [ ! -s /mnt/vendor/persist/hardware_debug ] ; then
    persistbdf=`md5sum /mnt/vendor/persist/bdwlanw.elf |cut -d" " -f1`
    vendorbdf=`md5sum /odm/etc/wifi/bdwlanw.elf |cut -d" " -f1`
...
chmod 666 /mnt/vendor/persist/bdwlanw.elf
chown system:wifi /mnt/vendor/persist/bdwlanw.elf
